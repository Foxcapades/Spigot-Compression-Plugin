...
internal object I18N : Observable() {
  ...
  inline fun fillLoreFor(lvl: CompressionLevel, name: String, locale: String = currentLocale) =
    loreTemplateFor(lvl, locale).replace("\${name}", name).replace("\${size}", lvl.count.toString())
  ...
  inline fun fillNameFor(lvl: CompressionLevel, name: String, locale: String = currentLocale) =
    nameTemplateFor(lvl, locale).replace("\${name}", name).replace("\${size}", lvl.count.toString())
  ...
  private fun tryLoadBundle(locale: String, fallback: Bundle): Bundle {
    ...
    tryLoadProperties(locale, BlocksFileName)?.let { blocks =
      io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(it, blocks)
    }
      ?: Logger.info("falling back to %s locale instead of %s for %s", DefaultLocale, locale, BlocksFileName)
    tryLoadProperties(locale, ItemsFileName)?.let { items = io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(it, items) }
    ...
    tryLoadProperties(locale, PluginFileName)?.let { plugin =
      io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(it, plugin)
    }
    ...
    tryLoadProperties(locale, StacksFileName)?.let { stacks =
      io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(it, stacks)
    }
  ...
  private fun loadDefaultBundle() =
    Bundle(
      blocks = io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(requireProperties(DefaultLocale, BlocksFileName)),
      items  = io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(requireProperties(DefaultLocale, ItemsFileName)),
      plugin = io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(requireProperties(DefaultLocale, PluginFileName)),
      stacks = io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode(requireProperties(DefaultLocale, StacksFileName)),
    )
...
private data class Bundle(
  val blocks: io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode,
  val items: io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode,
  val plugin: io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode,
  val stacks: io.foxcapades.bukkit.szip.i18n.LocalizationTreeNode,
)

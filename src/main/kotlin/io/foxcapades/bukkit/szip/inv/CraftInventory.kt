package io.foxcapades.bukkit.szip.inv

import io.foxcapades.bukkit.inventory.*
import io.foxcapades.bukkit.szip.ext.size
import io.foxcapades.bukkit.szip.zip.compressionLevel
import io.foxcapades.bukkit.szip.zip.ifCompressible
import io.foxcapades.bukkit.szip.item.Air
import io.foxcapades.bukkit.szip.zip.compressOld
...
internal value class CraftInventory(private val view: InventoryView) {
  ...
  fun popAll(): List<ItemStack> = run {
    ...
    ifAllTheSame { it, qty ->
      it.ifCompressible {
        ...
        out.add(it.compressOld(lvl.next, qty))
        reduceAllBy(qty)
      }
  ...
  private fun popAllSingular(it: ItemStack, out: MutableList<ItemStack>) {
    ...
    val decom = it.compressOld(lvl.previous, 0)
  ...
  fun putItem(index: Int, stack: ItemStack): ItemStack {
    ...
    if (slot.isSimilar(stack) && slot.hasSpace()) {
  ...
  fun fillGrid(stack: ItemStack, slotIndex: Int) {
    ...
    if (stack.amount < slots.size && stack.maxStackSize == 1) {
      ...
      tryFillGridFromHand(stack, slotIndex, slots)
    }
  ...
  fun calculateResult() {
    ifSingleStack {
      ...
      if (lvl.hasPrevious)
        top.setItem(0, it.compressOld(lvl.previous, 9))
    ...
    ifAllTheSame { it, _ ->
      it.ifCompressible {
        ...
        if (lvl.hasNext)
          top.setItem(0, it.compressOld(lvl.next, 1))
      }
  ...
  private fun tryFillGridFromHand(stack: ItemStack, slotIndex: Int, slots: List<AvailableSlot>) {
    val remainder = applySpread(stack, slots)

    // If there was no remainder, clear the slot and stop here.
    if (remainder === Air) {
      bottom[slotIndex] = null
      return
    }

    // Attempt to distribute the remainder into other stacks in the player's
    // inventory.
    val targets = ArrayList<ItemStack>(8)
    var leftToDistribute = remainder.size

    val it = findSuitableBottomInventorySlots(stack, slotIndex).iterator()

    while (it.hasNext()) {
      val (_, target) = it.next()

      leftToDistribute -= target!!.freeSpace
      targets.add(target)
    ...
    // If the full stack can't be distributed, then don't distribute anything.
    if (leftToDistribute > 0) {
      bottom[slotIndex] = remainder
      return
    }

    // Fill up the stacks we collected to distribute into.
    targets.forEach { it.amount = it.maxStackSize }

    // Handle negative remainder.
    targets.last().amount += leftToDistribute
  }
  ...
  private fun findSuitableBottomInventorySlots(toHold: ItemStack, excluding: Int) =
    (bottom.hotBarSlotSequence(reverse = true) + bottom.storageSlotSequence(reverse = true))
      .filter { (i, v) -> i != excluding && v.isNotEmpty() && v.hasSpace() && toHold.isSimilar(v) }
}

...
@JvmInline
internal value class CraftInventory(private val raw: Inventory) {
  ...
  fun calculateResult() {
    ifSingleStack {
      ...
      if (lvl.hasPrevious)
        raw.setItem(0, it.compress(lvl.previous, 9))
    ...
    ifAllTheSame { it, _ ->
      it.ifCompressible {
        ...
        if (lvl.hasNext)
          raw.setItem(0, it.compress(lvl.next, 1))
      }
